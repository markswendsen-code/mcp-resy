...
export async function login(params: {
  ...
  try {
    ...
    const isOnLoginPage =
      currentUrl.includes("/login") || pageContent.includes("incorrect password");
...
export async function searchVenues(params: {
  ...
  date?: string;
  partySize?: number;
}): Promise<{ success: boolean; venues?: object[]; error?: string }> {
  const p = await getPage();
  const ctx = await getContext();
  ...
  try {
    ...
    if (date) searchParams.set("day", date);
    ...
    const cards = p.locator(
      '[data-test-id="venue-card"], [class*="VenueCard"], [class*="venue-card"], [data-venue-id]'
    );
      ...
      try {
        ...
        const profileLink =
          (await card
            ...
            .catch(() => "")) || "";
        const venueIdAttr =
          (await card.getAttribute("data-venue-id").catch(() => "")) || "";
...
export async function getVenueDetails(venueId: string): Promise<{
  ...
  try {
    ...
    const name =
      (await p.locator("h1, [class*='venue-name'], [class*='VenueName']").first().textContent().catch(() => "")) || "";
    const description =
      (await p.locator('[class*="description"], [class*="Description"], [data-test-id="venue-description"]').first().textContent().catch(() => "")) || "";
    const cuisine =
      (await p.locator('[class*="cuisine"], [data-test-id="cuisine"]').first().textContent().catch(() => "")) || "";
    const address =
      (await p.locator('[class*="address"], address, [itemprop="address"]').first().textContent().catch(() => "")) || "";
    const phone =
      (await p.locator('[class*="phone"], a[href^="tel:"], [itemprop="telephone"]').first().textContent().catch(() => "")) || "";
    const priceRange =
      (await p.locator('[class*="price"], [aria-label*="price"]').first().textContent().catch(() => "")) || "";
    const ratingText =
      (await p.locator('[class*="rating"], [aria-label*="rating"]').first().textContent().catch(() => "")) || "";
    const neighborhood =
      (await p.locator('[class*="neighborhood"], [class*="Neighborhood"]').first().textContent().catch(() => "")) || "";
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to get venue details",
    };
  }
}
...
export async function checkAvailability(params: {
  ...
  time: string;
  partySize: number;
}): Promise<{ success: boolean; slots?: object[]; venueName?: string; error?: string }> {
  const p = await getPage();
  const ctx = await getContext();
  ...
  try {
    ...
    const venueName =
      (await p.locator("h1, [class*='venue-name']").first().textContent().catch(() => "")) || "Unknown Venue";
    ...
    const slots: object[] = [];
    const timeSlotEls = p.locator(
      '[class*="timeslot"], [class*="TimeSlot"], [data-test-id="time-slot"], button[data-config-id]'
    );
    ...
    for (let i = 0; i < Math.min(slotCount, 30); i++) {
      ...
      try {
        ...
        const configId =
          (await slot.getAttribute("data-config-id").catch(() => "")) || undefined;
        const slotType =
          (await slot.getAttribute("data-type").catch(() => "")) || undefined;
        if (timeText.trim()) {
          slots.push({
            time: timeText.trim(),
            date,
...
export async function makeReservation(params: {
  ...
  confirm: boolean;
}): Promise<{ success: boolean; reservation?: object; requiresConfirmation?: boolean; preview?: object; error?: string }> {
  const p = await getPage();
  const ctx = await getContext();
  ...
  try {
    ...
    const targetSlot = p.locator(
      `[class*="timeslot"]:has-text("${time}"), button[data-config-id]:has-text("${time}"), [data-test-id="time-slot"]:has-text("${time}")`
    ).first();
    ...
    if (await targetSlot.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const firstSlot = p.locator(
        '[class*="timeslot"], button[data-config-id], [data-test-id="time-slot"]'
      ).first();
    ...
    if (specialRequests) {
      const requestsField = p.locator(
        'textarea[name*="request"], textarea[placeholder*="request" i], [class*="special-request"] textarea'
      );
    ...
    const reserveButton = p.locator(
      'button:has-text("Reserve"), button:has-text("Book"), button:has-text("Complete"), button[type="submit"]'
    ).first();
...
export async function getReservations(): Promise<{
  ...
  try {
    ...
    const cards = p.locator(
      '[class*="reservation-card"], [class*="ReservationCard"], [data-test-id="reservation"]'
    );
    const count = await cards.count();
    ...
    for (let i = 0; i < Math.min(count, 30); i++) {
      ...
      try {
        const venueName =
          (await card.locator("h2, h3, [class*='venue-name']").first().textContent().catch(() => "")) || "";
        const dateText =
          (await card.locator("time, [class*='date'], [datetime]").first().textContent().catch(() => "")) || "";
        const timeText =
          (await card.locator("[class*='time'], [aria-label*='time']").first().textContent().catch(() => "")) || "";
        const partySizeText =
          (await card.locator("[class*='party'], [class*='guest'], [aria-label*='guest']").first().textContent().catch(() => "")) || "";
        const statusText =
          (await card.locator("[class*='status']").first().textContent().catch(() => "upcoming")) || "upcoming";
        const reservationId =
          (await card.getAttribute("data-reservation-id").catch(() => "")) || `res-${i}`;
...
export async function cancelReservation(params: {
  ...
  confirm: boolean;
}): Promise<{ success: boolean; requiresConfirmation?: boolean; message?: string; error?: string }> {
  const p = await getPage();
  const ctx = await getContext();
  ...
  try {
    ...
    const cancelButton = p.locator(
      'button:has-text("Cancel reservation"), button:has-text("Cancel"), [data-test-id="cancel-reservation"]'
    ).first();
    ...
    const confirmButton = p.locator(
      'button:has-text("Yes"), button:has-text("Confirm"), button:has-text("Yes, cancel")'
    ).first();
...
export async function getWaitlist(venueId: string): Promise<{
  success: boolean;
  waitlist?: object;
  error?: string;
}> {
  ...
  try {
    ...
    const waitlistEl = p.locator(
      '[class*="waitlist"], [class*="Waitlist"], [data-test-id="waitlist"], button:has-text("Join waitlist"), button:has-text("Waitlist")'
    ).first();

    const hasWaitlist = await waitlistEl.isVisible({ timeout: 5000 }).catch(() => false);

    let waitlistInfo: object = { available: false };
    if (hasWaitlist) {
      const waitlistText =
        (await waitlistEl.textContent().catch(() => "")) || "";
      const waitTimeEl = p.locator('[class*="wait-time"], [class*="WaitTime"]').first();
      const waitTime =
        (await waitTimeEl.textContent().catch(() => "")) || undefined;
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to get waitlist info",
    };
  }
}
...
export async function joinWaitlist(params: {
  ...
  partySize: number;
  confirm: boolean;
}): Promise<{ success: boolean; requiresConfirmation?: boolean; message?: string; waitlistEntry?: object; error?: string }> {
  const p = await getPage();
  const ctx = await getContext();
  ...
  try {
    ...
    const waitlistButton = p.locator(
      'button:has-text("Join waitlist"), button:has-text("Waitlist"), [data-test-id="join-waitlist"]'
    ).first();
    ...
    const partySizeSelector = p.locator(
      'select[name*="party"], select[name*="seats"], [class*="party-size"] select'
    ).first();
    ...
    // Confirm joining
    const confirmButton = p.locator(
      'button:has-text("Join"), button:has-text("Confirm"), button[type="submit"]'
    ).first();
